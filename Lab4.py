...
def buildGAN(images, epochs = 40000, batchSize = 32, loggingInterval = 0):
	...
	for epoch in range(epochs):
		...
		if loggingInterval > 0 and epoch % loggingInterval == 0:
			...
			plotLosses(d_loss, g_loss, epoch / loggingInterval)
...
def runGAN(generator, outfile):
	...
	final = np.clip(img, 0, 255).astype("uint8")
	imageio.imwrite(outfile, final)

def plotLosses(d_loss, g_loss, iteration):
	outfile = OUTPUT_DIR + "/plots.pdf"
...
def main():
	...
	(generator, adv, gan) = buildGAN(data, epochs = 60000, loggingInterval = 100)
